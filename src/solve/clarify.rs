...
pub fn clarify_iter(input: &[u8; 32]) -> Vec<[u8;32]> {
    let clarity = clarity_list();
    let mut result : Vec<[u8;32]> = vec![];

    let iter = input.iter()
        .map(|i| &clarity[*i as usize])
        .multi_cartesian_product();
    for v in iter {
        result.push(make_array(&v));
    }
    result
}

pub fn validate(input: &[u8; 32]) -> bool
{
    let clarity = clarity_list();
...
mod tests {
    ...
    fn test_inverse_iter() {
        for _ in 0 .. 50
        {
            ...
            for inverse in super::clarify_iter(&input) {
                let actual = crate::forward::confuse(&inverse);
                assert_eq!(input, actual);
            }
        }
